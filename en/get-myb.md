...
You can get the installation ISO image from the official website [under Downloads](https://myb.convectix.com/download/).
The installation uses the normal 'bsdinstall' FreeBSD installer and the MyBee installation procedure is no different from
normal [FreeBSD installation](https://docs.freebsd.org/en/books/handbook/bsdinstall/#bsdinstall-start) process.
...
# Installation via FreeBSD rescue (eg Hetzner Rescue System)

:bangbang: | :warning: A warning! These instructions destroy the information on the disks of the computer where MyBee is installed.
:---: | :---
...
The script is a sh script, so just run it from `/bin/sh`:
...
3) If you are familiar with the bsdinstaller, you will be familiar with the following steps. All further steps are similar to a normal FreeBSD installation. The only exception is that at the end of the script, you will return to the 'rescue' shell Hetzner - just reboot the server with the 'reboot' command yourself.

# MyB/FreeBSD installer

:information_source: If you are familiar with the FreeBSD installation process and the bsdinstall installer, you can [skip this chapter](shell.md).
...
MyB uses the rich features and stability of ZFS, the ability to use UFS is disabled:
...
In our example, we only have the two disks that were used as stripes in the previous step, with no high availability in case either disk fails:
...
Choose a strong password for the 'root' user. If you don't create an additional user in the next step, MyB will by default allow 'root' access remotely via SSH.
...
Without configuring DNS servers, MyB will not be able to get gold images from the repository. If you don't have preferred DNS servers, you can use public '8.8.8.8', '8.8.4.4' from Google:

![mybee_hz14](https://user-images.githubusercontent.com/926409/163675549-1417a25c-fff1-4189-b94c-743b97bc98fd.png)

Choose the most comfortable time zone for you:
...
You can create a privileged user to access the server remotely.
...
At this point, you've done everything to get MyB to continue with the installation, exit with bsdinstall.

![mybee_hz19](https://user-images.githubusercontent.com/926409/163675554-10af0f73-d95e-49d2-b041-0c61ef16c334.png)

Select 'no' to exit bsdinstall and, in the case of Hetzner, exit to the 'rescue' shell:

![mybee_hz20](https://user-images.githubusercontent.com/926409/163675558-72a96aca-b7cf-4c0a-97c7-23e719e09abd.png)

All you have to do is restart the server with the command: **shutdown -r now**

After reboots, you can use access to the server IP via SSH and to get start information, contact the server through a browser: http://IP
