...
Прежде чем вы сможете полноценно использовать Kubernetes кластера в MyB, вы должны указать поддомен, в котором будут создаваться кластера и на который будет ссылаться kubeconfig. Этот шаг необязателен, если вы испольуете внешние IP адреса, когда каждый control plane получает свой уникальный адрес.
Если вы не имеете дополнительных внешних адресов и не выполните эту настройку, вы будете ограничены лишь одним кластером kubernetes. Для настройки, используйте меню `17) Kubernetes clusters setting` -> `3) Change domain name for K8S (kubeconfig_domain: );`. Например, если в качестве kubeconfig_domain вы укажете домен `example.com`, то K8S кластера будут использовать DNS имена `env`.example.com, которые должны указывать на сервер MyB. Если вы не контроллируете DNS зону, вам придется добавлять соответствующие записи в /etc/hosts файле.
