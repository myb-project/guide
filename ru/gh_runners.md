...
- тем, у кого почти нет запусков в течении дня и кто хочет сэкономить на постоянно запущеном или выделенном сервера: арендуйте runner-ы в облаке по требованию и оплачивайте только время ее работы;
- тем, кто хочет кастомные ресурсы, уникальные flavor-ы, не зависеть от ограничений по кол-ву запуской и cputime в облаке GitHub, иметь большие мощности по себестоимости;

*MyBee* позволяет это реализовать, затратив ~10 минут на настройку. В свою очередь, это оказалось возможным благодаря поддержке cloud образов проекта CBSD, который используется MyBee в качестве примера переиспользования наработок CBSD для быстрого создания чего-то более комплексного.

Примечание1: Использование Windows ОС в качестве runner также возможно, но ввиду известных лицензионных ограничений, данный образ не поставляется с базовой версией MyBee (существует версия MyBee с оплаченными лицензиями на Windows)
Примечание2: Вы можете использовать облако/аренду MyBee от ConvectIX (оплата в ру, доллары, евро, биткоины) с размещением раннеров в различных регионах (Ру, UE)


>>  (или арендовав его за символическую наценку* (вы платите цену выбранного вами поставщика dedicated сервера - цена образовывается выбранным провайдером ). 
